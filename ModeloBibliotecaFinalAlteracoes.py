...
def atualizar_funcionarias(caminho_arquivo, DIAS_FUNCIONARIAS, FUNCIONARIAS):
    MAPA_TURNOS = {
        "SEG/TER": [0, 1],
        "QUA/QUI": [2, 3]
    }

    # Normaliza o dicionário FUNCIONARIAS para uppercase
    FUNCIONARIAS_UP = {nome.upper(): idx for nome, idx in FUNCIONARIAS.items()}

    # Reset
    for i in DIAS_FUNCIONARIAS:
        DIAS_FUNCIONARIAS[i] = [-1]

    with open(caminho_arquivo, "r", encoding="utf-8") as f:
        leitor = csv.reader(f)

        # Lê apenas as duas primeiras linhas
        for linha in [next(leitor), next(leitor)]:
            turno = linha[0].strip().upper()

            if turno not in MAPA_TURNOS:
                raise ValueError(f"Turno inválido: {linha[0]}")

            dias_turno = MAPA_TURNOS[turno]

            # A segunda coluna vem como UMA STRING só: "Gislene, Irene, Ana"
            bruto = linha[1].strip()

            # Remove aspas duplas se existirem
            bruto = bruto.replace('"', '')

            # Separa nomes pela vírgula
            nomes = [n.strip().upper() for n in bruto.split(",") if n.strip()]

            for nome in nomes:
                if nome not in FUNCIONARIAS_UP:
                    raise ValueError(f"Funcionária '{nome}' não existe no dicionário FUNCIONARIAS.")

                idx = FUNCIONARIAS_UP[nome]
                DIAS_FUNCIONARIAS[idx] = dias_turno[:]

    return DIAS_FUNCIONARIAS
...
FUNCIONARIAS = {
    ...
    "GISLENE": 3,   # Segunda e Terça
    # SAU
    "ANA": 4,       # Quarta e Quinta
    "IRENE": 5,     # Quarta e Quinta
    "REGINA": 6     # Segunda e Terça
}
...
DIAS_FUNCIONARIAS = atualizar_funcionarias("TabelaRestricao.csv", DIAS_FUNCIONARIAS, FUNCIONARIAS)
print(DIAS_FUNCIONARIAS)
G_seg_ter = [i for i, dias in DIAS_FUNCIONARIAS.items() if dias == [0, 1]]
G_qua_qui = [i for i, dias in DIAS_FUNCIONARIAS.items() if dias == [2, 3]]
print("Funcionárias Seg/Ter:", G_seg_ter)
print("Funcionárias Qua/Qui:", G_qua_qui)
...
def printa_escala(dia_inicial, mes_inicial, dia_final, mes_final, n_funcionarios,
                   ...
                   xs_val, xt_val, xqua_val, xqui_val, ano, arquivo_csv):
    """
    Versão corrigida:
     - usa DIAS_FUNCIONARIAS[funcionario] corretamente;
     - constrói um mapa (iso_year, iso_week) -> idx para semanas com feriado (exclui sábados isolados);
     - quando a semana atual tem feriado usa xs_val/xt_val/xqua_val/xqui_val[funcionario][idx];
     - quando não tem feriado usa o fallback antigo (conta_sextas in dias_trabalhados_sextas[funcionario]).
    """
    sabados_passados = 0
    sextas_passadas = 0
    feriados_passados = 0

    # --- monta mapa de semanas com feriado (ano ISO, semana ISO) -> índice 0..K-1 ---
    # exclui feriados que caem apenas no sábado (weekday()==5)
    semanas_com_feriado = []
    semanas_set = set()
    for f in feriados:
        d, m = f[0], f[1]
        try:
            ...
            continue
        if dt.weekday() == 5:  # sábado -> pular (mantendo a lógica que você já usa)
            continue
        iso_ano, iso_semana, _ = dt.isocalendar()
        key = (iso_ano, iso_semana)
        if key not in semanas_set:
            semanas_set.add(key)
            semanas_com_feriado.append(key)

    semanas_com_feriado.sort()  # ordena cronologicamente
    mapa_semana_para_idx = {wk: idx for idx, wk in enumerate(semanas_com_feriado)}
    # quant_semanas_feriados can be derived as len(semanas_com_feriado) if needed
    ...
    with open(arquivo_csv, 'w', encoding='utf-8') as csv_file:
        ...
        for mes in range(mes_inicial, mes_final + 1):
            ...
            for funcionario in range(n_funcionarios):
                ...
                # contadores de sábados/sextas (índices de ocorrência)
                conta_sabados = 1 + sabados_passados
                conta_sextas = 1 + sextas_passadas

                # itera dias do mês
                for dia in range(primeiro_dia, ultimo_dia + 1):
                    # se é feriado (dia, mes)
                    if any(f[0] == dia and f[1] == mes for f in feriados):
                        valor = "*"
                        dual_print("  * ", end="")
                    else:
                        dia_semana = calendar.weekday(ano, mes, dia)  # 0=seg ... 6=dom
                        ...
                        if dia_semana == 6:  # domingo
                            ...
                            if conta_sabados in dias_trabalhados_sabados.get(funcionario, []):
                                valor = "D"
                                dual_print("  D ", end="")
                            else:
                                valor = "*"
                            ...
                            if conta_sextas in dias_trabalhados_sextas.get(funcionario, []):
                                valor = "N"
                                dual_print("  N ", end="")
                            else:
                                valor = "D"
                            ...
                            # --- aqui: usa DIAS_FUNCIONARIAS[funcionario] corretamente ---
                            dias_da_func = DIAS_FUNCIONARIAS.get(funcionario, [-1])

                            if dia_semana in dias_da_func:
                                # descobrir se essa semana tem feriado (usando iso week)
                                try:
                                    wk = date(ano, mes, dia).isocalendar()
                                    chave = (wk[0], wk[1])  # (iso_year, iso_week)
                                except Exception:
                                    chave = None

                                idx_feriado = mapa_semana_para_idx.get(chave, None)

                                if idx_feriado is not None:
                                    # semana com feriado → usar xs/xt/xqua/xqui (se houver índice disponível)
                                    # proteção contra índice fora do alcance
                                    if dia_semana == 0:  # segunda
                                        if idx_feriado < len(xs_val[funcionario]):
                                            valor = "N" if xs_val[funcionario][idx_feriado] > 0.5 else "D"
                                        else:
                                            valor = "D"
                                        dual_print(f"  {valor} ", end="")

                                    elif dia_semana == 1:  # terça
                                        if idx_feriado < len(xt_val[funcionario]):
                                            valor = "N" if xt_val[funcionario][idx_feriado] > 0.5 else "D"
                                        else:
                                            valor = "D"
                                        dual_print(f"  {valor} ", end="")

                                    elif dia_semana == 2:  # quarta
                                        if idx_feriado < len(xqua_val[funcionario]):
                                            valor = "N" if xqua_val[funcionario][idx_feriado] > 0.5 else "D"
                                        else:
                                            valor = "D"
                                        dual_print(f"  {valor} ", end="")

                                    elif dia_semana == 3:  # quinta
                                        if idx_feriado < len(xqui_val[funcionario]):
                                            valor = "N" if xqui_val[funcionario][idx_feriado] > 0.5 else "D"
                                        else:
                                            valor = "D"
                                        dual_print(f"  {valor} ", end="")

                                else:
                                    # fallback: semana sem feriado → mantém lógica antiga (usa dias_trabalhados_sextas como critério)
                                    if conta_sextas in dias_trabalhados_sextas.get(funcionario, []):
                                        valor = "D"
                                        dual_print(f"  {valor} ", end="")
                                    else:
                                ...
                                # funcionária não trabalha neste dia da semana
                                valor = "D"
                                dual_print("  D ", end="")
            ...
            # atualiza contadores para o próximo mês com base no último funcionário processado
            feriados_passados = feriados_passados  # mantido por compatibilidade, mas não usado para indexar XS mais
...
def building_model():
    ...
    for i in range(n_funcionarios):
        if i == 0:
            model.addCons(alegria[i] == quicksum((pontos_folga*(1 - Y[i,d]) + asab[i][d] * Y[i,d]) for d in range(n_sabados)) / (pontos_folga*(n_sabados-math.floor(MSb_sti[-1])) + quicksum(a_sab[i][j] for j in range(math.floor(MSb_sti[-1])))))
        elif i == 2:
            model.addCons(alegria[i] == quicksum((pontos_folga*(1 - X[i,d]) + asex[i][d] * X[i,d]) for d in range(n_sextas)) / (pontos_folga*(n_sextas-math.floor(MSx[-1])) + quicksum(a_sex[i][j] for j in range(math.floor(MSx[-1])))))
        else:
            if i <= 3:
                model.addCons(alegria[i] == (quicksum((pontos_folga*(1 - X[i,d]) + asex[i][d]*X[i,d]) for d in range(n_sextas)) + quicksum((pontos_folga*(1 - Y[i,d]) + asab[i][d]*Y[i,d]) for d in range(n_sabados))) / (pontos_folga*(n_sextas-math.floor(MSx[-1])) + quicksum(a_sex[i][j] for j in range(math.floor(MSx[-1]))) + pontos_folga*(n_sabados-math.floor(MSb_sti[-1])) + quicksum(a_sab[i][j] for j in range(math.floor(MSb_sti[-1])))))
            else:
                model.addCons(alegria[i] == (quicksum((pontos_folga*(1 - X[i,d]) + asex[i][d]*X[i,d]) for d in range(n_sextas)) + quicksum((pontos_folga*(1 - Y[i,d]) + asab[i][d]*Y[i,d]) for d in range(n_sabados))) / (pontos_folga*(n_sextas-math.floor(MSx[-1])) + quicksum(a_sex[i][j] for j in range(math.floor(MSx[-1]))) + pontos_folga*(n_sabados-math.floor(MSb_sau[-1])) + quicksum(a_sab[i][j] for j in range(math.floor(MSb_sau[-1])))))
    ...
    model.setObjective(10 * MinAlegria + quicksum(alegria[i] for i in range(n_funcionarios)),"maximize")

    # Restrições

    # Sextas

    # Garante que Juliana não participem dos rodízios de sexta
    for d in range(n_sextas):
        model.addCons(X[0, d] == 0)

    # Garante que as funcionárias de mesmo turno não trabalhem juntas na sexta
    # Funcionárias Segunda/Terça = [0, 1]
    for d in range(n_sextas):
        model.addCons(quicksum(X[i, d] for i in G_seg_ter) <= 1)
    # Funcionárias Quarta/Quinta = [2, 3]
    for d in range(n_sextas):
        model.addCons(quicksum(X[i, d] for i in G_qua_qui) <= 1)

    # Garante dois funcionários por sexta
    for d in range(n_sextas):
        model.addCons(quicksum(X[i,d] for i in range(1, n_funcionarios)) == 2)

    # Garante que um funcionário não trabalhe três sextas consecutivas
    for i in range(1, n_funcionarios):
        for d in range(n_sextas - 1):
            model.addCons(X[i,d] + X[i, d + 1] <= 1)
    ...
    for i in range(n_funcionarios):
        for d in range(n_sabados - 1):
            if i != 2:
                model.addCons(Y[i,d] + Y[i, d + 1] <= 1)
    ...
    for mes in range(num_meses_sabados - 1): # Evita `mes + 1` fora do limite
        for i in range(n_sti):
            if i != 2:
                model.addCons(math.floor((quant_sabados_mes[mes]+quant_sabados_mes[mes+1])/(n_sti-1)) <= quicksum(Y[i,matriz_indice_sabados[mes][j]] for j in range(quant_sabados_mes[mes])) + quicksum(Y[i,matriz_indice_sabados[mes+1][j]] for j in range(quant_sabados_mes[mes+1])))
                model.addCons(quicksum(Y[i,matriz_indice_sabados[mes][j]] for j in range(quant_sabados_mes[mes])) + quicksum(Y[i,matriz_indice_sabados[mes+1][j]] for j in range(quant_sabados_mes[mes+1])) <= math.ceil((quant_sabados_mes[mes]+quant_sabados_mes[mes+1])/(n_sti-1)))
    ...
    for d in range(n_sabados):
        model.addCons(quicksum(Y[i, d] for i in range(n_sti) if i != 2) == 1)

    for d in range(n_sabados):
        model.addCons(quicksum(Y[i,d] for i in range(n_sti,n_funcionarios)) == 1)

    # Garante que a Bia não trabalhe sábado
    for d in range(n_sabados):
        model.addCons(Y[2, d] == 0)
...
def feriados_model():
    ...
    for i in range(n_funcionarios):
        for s in range(quant_semanas_feriados):
            XS[(i, s)] = model.addVar(vtype="BINARY", name="XT({},{})".format(i,s))
    ...
    for i in range(n_funcionarios):
        for s in range(quant_semanas_feriados):
            XQua[(i, s)] = model.addVar(vtype="BINARY", name="XQ({},{})".format(i,s))
    
    for i in range(n_funcionarios):
        for s in range(quant_semanas_feriados):
            XQui[(i, s)] = model.addVar(vtype="BINARY", name="XT({},{})".format(i,s))
    ...
    for semana in range(quant_semanas_feriados):
        ...
        if 0 in feriados_matriz[semana]:
            for funcionario in range(n_funcionarios):
                model.addCons(XS[funcionario, semana] == 0)
            feriados_segundas.append(semana)
        ...
        if 2 in feriados_matriz[semana]:
            for funcionario in range(n_funcionarios):
                model.addCons(XQua[funcionario, semana] == 0)
        ...
        if 3 in feriados_matriz[semana]:
            for funcionario in range(n_funcionarios):
                model.addCons(XQui[funcionario, semana] == 0)
            feriados_quintas.append(semana)
    ...
    # Funcionárias - Segunda e Terça
    for i in G_seg_ter:
        model.addCons(
            Zmin <= quicksum(XS[i, semana] for semana in range(quant_semanas_feriados)) +
                    quicksum(XT[i, semana]  for semana in range(quant_semanas_feriados)) +
                    len(sextas_em_feriados[i])
                )

    # Funcionarias - Quarta e Quinta
    for i in G_qua_qui:
        model.addCons(
            Zmin <= quicksum(XQua[i, semana] for semana in range(quant_semanas_feriados)) +
                    quicksum(XQui[i, semana]  for semana in range(quant_semanas_feriados)) +
                    len(sextas_em_feriados[i])
                )
    ...
    for i in G_seg_ter:
        model.addCons(
            Zmax >= quicksum(XS[i, semana] for semana in range(quant_semanas_feriados)) +
                    quicksum(XT[i, semana]  for semana in range(quant_semanas_feriados)) +
                    len(sextas_em_feriados[i])
                )
    ...
    for i in G_qua_qui:
        model.addCons(
            Zmax >= quicksum(XQua[i, semana] for semana in range(quant_semanas_feriados)) +
                    quicksum(XQui[i, semana]  for semana in range(quant_semanas_feriados)) +
                    len(sextas_em_feriados[i])
                )
    ...
    for semana_atual in range(quant_semanas_feriados):
        if semana_atual not in feriados_segundas:
            model.addCons(quicksum(XS[i, semana_atual] for i in G_seg_ter) == 2)

        if semana_atual not in feriados_tercas:
            model.addCons(quicksum(XT[i, semana_atual] for i in G_seg_ter) == 2)

        if semana_atual not in feriados_quartas:
            model.addCons(quicksum(XQua[i, semana_atual] for i in G_qua_qui) == 2)

        if semana_atual not in feriados_quintas:
            model.addCons(quicksum(XQui[i, semana_atual] for i in G_qua_qui) == 2)

    # Faz o funcionário trabalhar apenas dois dias a noite por semana
    for semana in range(quant_semanas_feriados):
        # Funcionárias - Segunda e Terça
        for i in G_seg_ter:
            model.addCons(XS[i, semana] + XT[i, semana] + bool_esta_sexta(sextas_em_feriados, semana, i) <= 2)
        
        # Funcionarias - Quarta e Quinta
        for i in G_qua_qui:
            model.addCons(XQua[i, semana] + XQui[i, semana] + bool_esta_sexta(sextas_em_feriados, semana, i) <= 2)
    ...
    for semana in range(quant_semanas_feriados):
        # Juliana(0)
        model.addCons(XS[0, semana] == 0)
        model.addCons(XT[0, semana] == 0)
        model.addCons(XQua[0, semana] == 0)
        model.addCons(XQui[0, semana] == 0)
        
        # Funcionárias - Segunda e Terça
        for i in G_seg_ter:
            model.addCons(XQua[i, semana] == 0)
            model.addCons(XQui[i, semana] == 0)

        # Funcionarias - Quarta e Quinta
        for i in G_qua_qui:
            model.addCons(XS[i, semana] == 0)
            model.addCons(XT[i, semana] == 0)
...
try:
    with open(file_name, 'r', encoding='utf-8') as file:
        ...
        n_sti = 4
        n_sau = 3
        n_noturnos = n_funcionarios - 1 # Quantidade de funcionários da escala noturna
       ...
       # Criar dicionários para armazenar os dias trabalhados de cada funcionário
        ...
        xs_val = [[0] * quant_semanas_feriados for _ in range(n_funcionarios)]
        xt_val = [[0] * quant_semanas_feriados for _ in range(n_funcionarios)]
        xqua_val = [[0] * quant_semanas_feriados for _ in range(n_funcionarios)]
        xqui_val = [[0] * quant_semanas_feriados for _ in range(n_funcionarios)]
        ...
        if model.getStatus() == "optimal":
            ...
            if model_feriados.getStatus() == "optimal":
                ...
                for i in range(n_funcionarios):
                    for s in range(quant_semanas_feriados):
                        xs_val[i][s] = model_feriados.getVal(XS[(i, s)])
                        xt_val[i][s] = model_feriados.getVal(XT[(i, s)])
                        xqua_val[i][s] = model_feriados.getVal(XQua[(i, s)])
                        xqui_val[i][s] = model_feriados.getVal(XQui[(i, s)])
                        print(f"XS({i},{s}) = {xs_val[i][s]} \t XT({i},{s}) = {xt_val[i][s]} \t XQ({i},{s}) = {xqua_val[i][s]} \t XQi({i},{s}) = {xqui_val[i][s]}")
            ...
            printa_escala(primeiro_dia[0], primeiro_dia[1], ultimo_dia[0], ultimo_dia[1], n_funcionarios, dias_trabalhados_sabados, dias_trabalhados_sextas, dias_sem_trabalho, xs_val, xt_val, xqua_val, xqui_val, ano, file_name)
